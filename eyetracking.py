...
def draw_heatmap(x_average, y_average):
    ...
    if 0 <= x_average < SCREEN_WIDTH and 0 <= y_average < SCREEN_HEIGHT:
        heatmap[x_average - 10: x_average + 10, y_average - 10: y_average + 10] += 1
    ...
    for x in range(0, SCREEN_WIDTH, 10):
        for y in range(0, SCREEN_HEIGHT, 10):
            alpha = min(heatmap[x, y] / max_val, 1)
            color_index = int(alpha * (len(HEATMAP_COLORS) - 1))
...
while True:
    ...
    draw_point(x_average, y_average, POINT_COLOR)
    pygame.display.flip()
